...
def get_output_relu2(self, input, output):
    if train_count == 100:
        ...
        plt.hist(a, bins=20, color='green')
        #plt.axis([-0.2, 0.2, 0, 2])
        plt.savefig('part3_5_relu2.png')

def get_output_bn2(self, input, output):
    if train_count == 100:
        a = output.data[10,:,:,:].reshape([1,14*14*16]).squeeze().cpu().numpy()
        plt.clf()
        plt.hist(a, bins=20, color='green')
        #plt.axis([-0.2, 0.2, 0, 2])
        plt.savefig('part3_5_bn2.png')

model.features.conv2.register_forward_hook(get_output_conv2)
model.features.relu2.register_forward_hook(get_output_relu2)
model.features.bn2.register_forward_hook(get_output_bn2)
